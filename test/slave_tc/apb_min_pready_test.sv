`ifndef APB_MIN_PREADY_TEST
`define APB_MIN_PREADY_TEST

class apb_min_pready_test extends apb_base_test;

    `uvm_component_utils(apb_min_pready_test)

    apb_min_pready_seqs pready_seqs[];

    function new(string name = "apb_min_pready_test", uvm_component parent = null);
    ...
    task run_phase(uvm_phase phase);
        ...
        foreach(pready_seqs[i])
            pready_seqs[i] = apb_min_pready_seqs :: type_id :: create($sformatf("pready_seqs_%0d", i));
        fork
            begin

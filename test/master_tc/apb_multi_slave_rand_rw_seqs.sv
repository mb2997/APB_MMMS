`ifndef APB_MULTI_SLAVE_RAND_RW_SEQS
`define APB_MULTI_SLAVE_RAND_RW_SEQS

class apb_multi_slave_rand_rw_seqs extends apb_master_seqs;

    `uvm_object_utils(apb_multi_slave_rand_rw_seqs)

    function new (string name = "apb_multi_slave_rand_rw_seqs");
        super.new(name);
    endfunction
    ...
    task body();
        ...
        repeat(no_of_trans) begin
            ...
            $display("\n---------- apb_multi_slave_rand_rw_seqs transaction : %0d ----------\n", apb_master_trans::current_trans_m);
